"""
Iterative spectral merging for the Conquer phase — *original* method.

A faithful port of ``divide_and_conquer/iterative_merging.py``: cosine
similarity (normalised dot product) on cluster-mean features only.  No RBF
kernel and no adaptive/quantile thresholds — those are extensions used by the
other variants.
...
def _merged_clusters(i: int, j: int, clusters: list[dict]) -> dict:
...
def iterative_merge(
    ...
    Adjacent patches whose cosine similarity exceeds *thresh* are merged
    iteratively.  The process runs for each threshold in *threshes* in sequence
    (continuing from where the previous threshold left off), producing one
    coarser set of cluster masks per level.

    Parameters
    ----------
    features : (H, W, C) torch.Tensor of patch features.
    threshes : decreasing absolute cosine cutoffs, e.g. ``[0.6, 0.5, …, 0.1]``.
    min_size : minimum patch count for a cluster to appear in the output.

    Returns
    -------
    List of ``(K, H, W)`` arrays, one per threshold level.
    """
    clusters: list[dict] = []
    ...
    for thresh in threshes:
        while len(similarities):
            ...
            merged = _merged_clusters(i, j, clusters)
        ...
        single_level_masks: list[np.ndarray] = []
        counted_cluster: set[int] = set()
